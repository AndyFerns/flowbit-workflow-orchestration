...
async function triggerN8nWorkflow(workflowId: string, inputPayload?: any) {
  ...
  try {
    ...
    // Customize body to send inputPayload if provided, else empty or fallback
    const body = inputPayload ? JSON.stringify({ input: inputPayload }) : undefined
...
async function triggerLangflowWorkflow(flowId: string, inputPayload?: any) {
  ...
  try {
    ...
    // Use dynamic inputPayload if provided, else fallback to static defaults
    const bodyPayload = inputPayload ?? {
      input_value: "Manual trigger from FlowBit Dashboard",
      input_type: "chat",
      output_type: "chat",
    }
...
export async function POST(request: Request) {
  try {
    const { workflowId, engine, inputPayload } = await request.json()
